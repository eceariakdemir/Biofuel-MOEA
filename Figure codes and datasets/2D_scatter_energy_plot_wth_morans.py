...
for b in billion:
    ...
    for v in version:
        ...
        tc = df_ha.iloc[:,0:107]
        tg = df_ha.iloc[:,107:214]
        tg.columns = tc.columns
        ta = df_ha.iloc[:,214:321]
        ta.columns = tc.columns
        tgl = df_ha.iloc[:,321:428]
        tgl.columns = tc.columns
        tal = df_ha.iloc[:,428:]
        tal.columns = tc.columns
        ...
        cb_ax.set_ylabel('Morans I Value',fontsize=14, rotation=270, labelpad = 15)
