...
#### To create each mep option see OPTION PART line 220
...
df_geo_grass_AG = pd.read_excel('Platypus_codes/combined_pivot_AG_Switchgrass.xlsx',header=0, engine='openpyxl') 
df_geo_grass_ML = pd.read_excel('Platypus_codes/combined_pivot_ML_Switchgrass.xlsx',header=0, engine='openpyxl') 
df_geo_algae_AG = pd.read_excel('Platypus_codes/combined_pivot_AG_Algae.xlsx',header=0, engine='openpyxl')   
df_geo_algae_ML = pd.read_excel('Platypus_codes/combined_pivot_ML_Algae.xlsx',header=0, engine='openpyxl')  


# Greenhouse gas emission : contains State - STASD_N - greenhouse gas emission (gCO2/MJ) (1998-2013)
df_geo_corn_GHG= pd.read_excel('Platypus_codes/Corn_GHG.xlsx',header=0, engine='openpyxl') 
df_geo_soy_GHG = pd.read_excel('Platypus_codes/Soy_GHG.xlsx',header=0, engine='openpyxl') 
df_geo_grass_AG_GHG = pd.read_excel('Platypus_codes/Switchgrass_AG_GHG.xlsx',header=0, engine='openpyxl')
df_geo_grass_ML_GHG = pd.read_excel('Platypus_codes/Switchgrass_ML_GHG.xlsx',header=0, engine='openpyxl') 
df_geo_algae_AG_GHG = pd.read_excel('Platypus_codes/Algae_AG_GHG.xlsx',header=0, engine='openpyxl')
df_geo_algae_ML_GHG = pd.read_excel('Platypus_codes/Algae_ML_GHG.xlsx',header=0, engine='openpyxl')


# Minimum fuel selling price (MFSP) : contains State - STASD_N - MFSP ($/MJ) (1998-2013)
df_geo_corn_MFSP = pd.read_excel('Platypus_codes/Corn_MFSP.xlsx',header=0, engine='openpyxl')
df_geo_soy_MFSP = pd.read_excel('Platypus_codes/Soy_MFSP.xlsx',header=0, engine='openpyxl')
df_geo_grass_AG_MFSP = pd.read_excel('Platypus_codes/Switchgrass_AG_MFSP.xlsx',header=0, engine='openpyxl') 
df_geo_grass_ML_MFSP = pd.read_excel('Platypus_codes/Switchgrass_ML_MFSP.xlsx',header=0, engine='openpyxl')
df_geo_algae_AG_MFSP = pd.read_excel('Platypus_codes/Algae_AG_MFSP.xlsx',header=0, engine='openpyxl')
df_geo_algae_ML_MFSP = pd.read_excel('Platypus_codes/Algae_ML_MFSP.xlsx',header=0, engine='openpyxl')
...
total_GHG_c = np.zeros((len(district))) # creating empty list for 106 total yield data set 
dist_mean_GHG_c = np.zeros((len(district))) # creating empty list for 106 mean yield data set 
total_GHG_s = np.zeros((len(district))) # creating empty list for 106 total yield data set 
dist_mean_GHG_s = np.zeros((len(district))) # creating empty list for 106 mean yield data set 
total_GHG_g = np.zeros((len(district))) # creating empty list for 106 total yield data set 
dist_mean_GHG_g = np.zeros((len(district))) # creating empty list for 106 mean yield data set
total_GHG_a = np.zeros((len(district))) # creating empty list for 106 total yield data set 
dist_mean_GHG_a = np.zeros((len(district))) # creating empty list for 106 mean yield data set
...
for dist in district:
    ...
    C_GHG = sum(Corn_emission[i,:])
    total_GHG_c[i] = C_GHG
    mean_energy_GHG_c = C_GHG/(len(years))
    dist_mean_GHG_c[i] = mean_energy_GHG_c
...
for dist in district:
    ...
    S_GHG = sum(Soy_emission[i,:])
    total_GHG_s[i] = S_GHG 
    mean_energy_GHG_s = S_GHG/(len(years))
    dist_mean_GHG_s[i] = mean_energy_GHG_s
...
for dist in district:
    ...
    G_GHG = sum(Grass_emission_ML[i,:])
    total_GHG_g[i] = G_GHG 
    mean_energy_g = G_GHG/(len(years))
    dist_mean_GHG_g[i] = mean_energy_g
...
for dist in district:
    ...
    A_GHG = sum(Algae_emission_ML[i,:])
    total_GHG_a[i] = A_GHG 
    mean_energy_a = A_GHG/(len(years))
    dist_mean_GHG_a[i] = mean_energy_a

## OPTIONS
### Each option can be created one by one. Also center color can be changed by changing vcenter value line 263
tc = dist_mean_c
# tc = dist_mean_s
# tc = dist_mean_g
# tc = dist_mean_a
...
# tc = land_limits
# tc = marginal_land_limits_grass
# tc = marginal_land_limits_algae
# tc = electricity
# tc = dist_mean_en_c
# tc = dist_mean_en_s
# tc = dist_mean_en_g
